...
Kubernetes operator (in Rust, on `kube-rs`) that reconciles `N8nInstance` custom resources
into running [n8n](https://n8n.io) deployments. Modelled on
[`kube-rs/controller-rs`](https://github.com/kube-rs/controller-rs).

## CRD

`N8nInstance` (`n8n.slys.dev/v1`, namespaced, shortname `n8n`) — describes a single n8n
