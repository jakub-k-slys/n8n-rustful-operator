...
Kubernetes operator written in Rust on top of `kube-rs` (v3). Reconciles `Single`
...
- `src/lib.rs` — re-exports `controller::*` (so `Single`, `State`, `run` are crate-root),
  defines the `Error` enum used as the reconciler's error type.
- `src/controller.rs` — the whole reconciler:
  - `Single` / `SingleSpec` / `SingleStatus` (the CRD, via `#[derive(CustomResource)]`).
  - `reconcile` wraps the user logic in `kube::runtime::finalizer` with `N8N_FINALIZER`.
    The finalizer guarantees `Single::cleanup` runs on delete.
  - `Single::reconcile` applies the child `Deployment` + `Service` (`build_deployment`,
...
- `src/metrics.rs` — Prometheus registry (`n8n_operator_reconcile` prefix). `ReconcileMeasurer`
  ...
  `Error::IllegalSingle` and is used in tests to assert the failure path.
...
- `src/crdgen.rs` — second binary; prints the CRD YAML produced by `Single::crd()`.

### CRD naming

Group `n8n.slys.dev`, kind `Single`, plural `instances` (overridden in the `#[kube(...)]`
...
The finalizer string `singles.n8n.slys.dev` mirrors the CRD name.

### Adding fields to the CRD

1. Edit `SingleSpec` / `SingleStatus` in `src/controller.rs`.
...
4. If you added a status field, also update the `Patch::Apply(json!({...}))` block in
   `Single::reconcile` — server-side apply will drop fields not present in the patch.
